...
as well as their schemas is available in the [CODEBOOK](CODEBOOK.md).
...
where each stage is responsible for generating and validting specific table(s)
...
* Each company record is marked as either a valid record or outlier
  ...
  the a custom outlier detection algorithm based on the idea that
...
3. For each record, calculate the ratio of its productivity to the average
   ...
   For records with values outside of the range $\pm \lambda$ around
   ...
   * $\lambda$ is a parameter defined in the [params.yaml](params.yaml)
   file for separately for each stage implementing outlier detection.
   The default values is ``0.5``, which corresponds to the assumption
